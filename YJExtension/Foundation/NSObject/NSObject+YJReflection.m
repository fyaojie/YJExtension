...
@implementation NSObject (YJReflection)
...
{
    return [[self class] yj_propertyKeys];
}
+ (NSArray *)yj_propertyKeys {
...
{
    return [[self class] yj_propertiesInfo];
}
/**
...
+ (NSArray *)yj_propertiesInfo
{
    ...
    for (int i = 0; i < propertyCount; i++)
    {
        [propertieArray addObject:({
            ...
            NSDictionary *dictionary = [self yj_dictionaryWithProperty:properties[i]];
...
+ (NSArray *)yj_propertiesWithCodeFormat
{
    ...
    NSArray *properties = [[self class] yj_propertiesInfo];
...
-(NSArray*)yj_methodListInfo{
    ...
    for (int i = 0; i < count ; i++)
    {
        ...
        for (int index=0; index<argumentsCount; index++) {
            ...
            [arguments addObject:[[self class] yj_decodeType:arg]];
        }
        
        NSString *returnTypeString =[[self class] yj_decodeType:returnType];
        NSString *encodeString = [[self class] yj_decodeType:encoding];
...
-(NSDictionary *)yj_protocolList{
    return [[self class]yj_protocolList];
}
+ (NSDictionary *)yj_protocolList
{
...
+ (NSArray *)yj_instanceVariable
{
    ...
    NSMutableArray *result = [NSMutableArray array];
    for (int i = 0; i < outCount; i++) {
        NSString *type = [[self class] yj_decodeType:ivar_getTypeEncoding(ivars[i])];
...
#pragma mark -- help
+ (NSDictionary *)yj_dictionaryWithProperty:(objc_property_t)property
{
...
+ (NSString *)yj_decodeType:(const char *)cString
{
    ...
    } else
    {
        if ([[result substringToIndex:1] isEqualToString:@"^"]) {
            result = [NSString stringWithFormat:@"%@ *",
                      [NSString yj_decodeType:[[result substringFromIndex:1] cStringUsingEncoding:NSUTF8StringEncoding]]];
        }
    }
    return result;
}
