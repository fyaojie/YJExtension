//
//  YJUIKit.h
//  YJExtensionDemo
//
//  Created by cool on 2018/6/7.
//  Copyright © 2018 child. All rights reserved.
//

#ifndef YJUIKit_h
#define YJUIKit_h

#import "UIWebView+YJExtension.h" // 常用方法
#import "UIWebView+YJSwipeGesture.h" // 返回前进手势

#import "UIResponder+YJUpdate.h" // 跳转到itunes某个id的程序

#import "UIImageView+YJExtension.h"
#import "UIImageView+YJReflect.h" // 倒影 

#import "UITextField+YJExtension.h"
#import "UITextField+YJInputLimit.h" // 内容输入长度限制
#import "UITextField+YJSelect.h" // 内容选中

#import "UIApplication+YJExtension.h" // 整个程序所占内存

#import "UIImage+YJSize.h" // 通过网络地址获取图片尺寸
#import "UIImage+YJGIF.h" // GIF
#import "UIImage+YJAlpha.h" // 透明度
#import "UIImage+YJColor.h" // 颜色生成图片
#import "UIImage+YJCompress.h" // 图片压缩
#import "UIImage+YJOrientation.h" // 图片方向纠正
#import "UIImage+YJExtension.h" // 图片处理
#import "UIImage+YJWatermark.h" // 水印

#import "UIWindow+YJExtension.h" // 层次结构

#import "UILabel+YJExtension.h"
#import "UILabel+YJSpace.h" // 文字间距
#import "UILabel+YJAutomaticWriting.h" // 自动书写打字

#import "UIButton+YJBadge.h" // 角标
#import "UIButton+YJCountDown.h" // 倒计时
#import "UIButton+YJEnlargeEdge.h" // 热区
#import "UIButton+YJImagePosition.h" // 图片和文字的位置
#import "UIButton+YJSubmitting.h" // 菊花按钮

#import "UIColor+YJExtension.h" // 颜色常用方法

#import "UIView+YJFrame.h" // 图片尺寸
#import "UIView+YJGestureRecognizer.h" // 点击长按手势
#import "UIView+YJNib.h" // nib文件获取
#import "UIView+YJExtension.h" // 截图 所在vc

#import "UITextView+YJPlaceHolder.h" // 默认文字
#import "UITextView+YJExtension.h" // 内容输入长度限制，内容选中，捏合手势控制字体大小

#import "UIDevice+YJExtension.h" // 设备相关

#import "UIControl+YJExtension.h" // 声音，事件触发

#import "UIBarButtonItem+YJBadge.h" // 角标
#import "UIBarButtonItem+YJExtension.h" // 导航按钮创建

#import "UINavigationController+YJStackManager.h" // 栈管理
#import "UINavigationController+YJExtension.h" // 过渡动画

#endif /* YJUIKit_h */
